...
app.get("/",(req,res)=>{
    res.cookie("pagevisited","home",{maxAge:1000*60*60,httpOnly:true,secure:true,path:"/"})
    res.cookie("prod","products",{maxAge:1000*60*60,httpOnly:true,secure:true,path:"/products"})
    res.json({
...
app.get("/signin", (req, res) => {
  ...
  app.get("/verify",async(req,res)=>{
        try{
            ...
            if(token){
                ...
                res.json({
                    message:"Token expired!!! Please login",data:null
                })
            }
        }catch(err){
            console.log(err)
  ...
  app.post("/login",async (req,res)=>{
    ...
    if(!user){
        ...
        console.log("user password",user.password,"recieved password",password);
       if(password== undefined || password==null ){
        ...
        if(user.password!=password){
            ...
            const token = jwt.sign({data:user},secret_key,{expiresIn:"1hr"});
